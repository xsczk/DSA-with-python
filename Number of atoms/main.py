...
class Solution:
    # stack
    # time complexity: O(n^2) as we have to revisit the nested formula to update it
        ...
        while i < len(formula):
            ...
            # multiply the count with the multiplicity of the nested formula
                ...
                while i < len(formula) and formula[i].isdigit():
                    ...
                    i += 1
                if multiplier:
                    for atom in top_formula:
                        top_formula[atom] *= int(multiplier)
                for atom in top_formula:
                    stack[-1][atom] += top_formula[atom]
            ...
            # atom with frequency, and update the most recent hashmap
                ...
                if cur_count == '':
                    ...
                    stack[-1][cur_atom] += int(cur_count)
        # sort the top hash map
        atom_count_dict = dict(sorted(stack[0].items()))
        return ''.join(
            [k + (str(v) if v > 1 else '') for k, v in atom_count_dict.items()])
