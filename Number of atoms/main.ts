...
function countOfAtoms(formula: string): string {
   ...
   while (i < formula.length) {
      if (formula[i] == '(') {
         ...
         const top_formula = stack.pop()
         let multiplier = ''
         i += 1
         while (i < formula.length && isDigits(formula[i])) {
            multiplier += formula[i]
            i += 1
         }
         if (multiplier) {
            for (const atom in top_formula) {
               top_formula[atom] *= Number(multiplier)
            }
         }
         for (const atom in top_formula) {
            stack.at(-1)![atom] = (stack.at(-1)![atom] ?? 0) + top_formula[atom]
         }
      } else {
         let atom = formula[i]
         i += 1
         ...
         if (!count) {
            stack.at(-1)![atom] = (stack.at(-1)![atom] ?? 0) + 1
         } else {
            stack.at(-1)![atom] = (stack.at(-1)![atom] ?? 0) + Number(count)
         }
      }
   }
   const atom_obj = Object.entries(stack[0])
   return atom_obj.sort((a, b) => a[0].localeCompare(b[0]))
       .reduce((res,
                cur) => res += cur[0] + (cur[1] > 1 ? `${cur[1]}` : ''), '')
}
