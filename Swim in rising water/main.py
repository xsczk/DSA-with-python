...
the elevation of both squares individually are at most t. You can swim infinite distances
...
class Solution:
    def swim_in_water(self, grid: list[list[int]]) -> int:
        ...
        while priority_queue:
            ...
            t, i, j = heapq.heappop(priority_queue)
            if i == j == n - 1:
                return t

            for di, dj in [(1, 0), (-1, 0), (0, 1), (0, -1)]:
                ni, nj = di + i, dj + j
                if ni in range(n) and nj in range(n) and (ni,
                                                          nj) not in visited:
                    t_new = max(t, grid[ni][nj])
                    heapq.heappush(priority_queue, (t_new, ni, nj))
                    visited.add((ni, nj))
