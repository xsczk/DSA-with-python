...
Each time a function starts or ends, we write a log with the ID, whether it started or ended,
and the timestamp.
...
Function 0 (initial call) resumes execution at the beginning of time 7
...
Function 1 starts at the beginning of time 6, executes 1 unit of time,
and ends at the end of time 6.
...
class Solution:
    # stack, keep track of the previous function process
    # time complexity: O(n)
    # space complexity: O(n) - stack size
    def exclusiveTime(self, n: int, logs: list[str]) -> list[int]:
        res = [0] * n
        # stack store the function id for each call (include recursion)
        stack: list[int] = []
        # keep track of the previous process start timestamp
        prev = 0
        for log in logs:
            func_id, status, timestamp = log.split(":")
            curr_timestamp = int(timestamp)
            if status == "start":
                # if there's a function running on top of the stack,
                # add current timestamp - prev to its exclusive time
                if stack:
                    res[stack[-1]] += curr_timestamp - prev
                prev = curr_timestamp
                stack.append(int(func_id))
            # end of the function call, add current timestamp - prev + 1
            else:
                stack.pop()
                res[int(func_id)] += curr_timestamp - prev + 1
                prev = curr_timestamp + 1
        return res
